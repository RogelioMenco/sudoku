...
var tablaSudoku = generaTabla(3, 3, generaTabla(3, 3, '<input maxlength="1" class="entrada_sudoku" onblur="verificarNumero(this)">').outerHTML);
...
function llenarTabla(matrizSudoku) {
    ...
    for (var i = 0; i < tablaSudoku.rows.length; i++) {
        ...
        for (var j = 0; j < tablaSudoku.rows[i].cells.length; j++) {
             ...
             */
            llenarCuadricula(obtenerCuadrante(matrizSudoku, i, j), tablaSudoku.rows[i].cells[j].firstElementChild)
        }
    }
}
    ...
    for (var i = 0; i < cuadricula.rows.length; i++) {
        ...
        for (var j = 0; j < cuadricula.rows[i].cells.length; j++) {
            ...
            if (contenido && contenido[i][j] != 0) {
                cuadricula.rows[i].cells[j].firstElementChild.setAttribute("disabled", "disabled"); //Evita cambios en la casilla
...
function tablaAMatriz() {
    ...
    for (var i = 0; i < tablaSudoku.rows.length; i++) {
        ...
        //Recorre las celdas de una fila
        for (var j = 0; j < tablaSudoku.rows[i].cells.length; j++) {
            tmpMatriz.push(cuadriculaAMatriz(tablaSudoku.rows[i].cells[j].firstElementChild))
        }
    }
...
function convertirMatrizCuadrantes(matriz) {
    ...
    for (j = 0; j < 3; j++) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            tempFila = matriz[indiceCuadricula][i].concat(matriz[indiceCuadricula + 1][i]).concat(matriz[indiceCuadricula + 2][i]);
